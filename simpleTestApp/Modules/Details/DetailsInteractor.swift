...
enum DetailsCellModel: Hashable {
    case top(carInfo: AutoInfoResponse)
    case bottom(post: Post)
}
...
final class DetailsInteractor {
    ...
    private func fetchData() {
        ...
        fetchTopSectionData { [weak self] data in
            self?.topCells = .top(carInfo: data)
            self?.dispatch.leave()
        }
        
        fetchBottomSectionData { [weak self] data in
            
            self?.bottomCells = data.posts.map({ DetailsCellModel.bottom(post: $0) })
            self?.dispatch.leave()
        }
    ...
    private func appendToPaginatedBottomCategory(data: AutoPostsResponse) {
        bottomCells?.append(contentsOf: data.posts.map({ DetailsCellModel.bottom(post: $0) }))
        self.presenter?.reloadCollection()
    }
}
